...
static const int mvebu_comphy_modes[MVEBU_COMPHY_LANES][COMPHY_MAX] = {
	{	/* lane 0 */
		[COMPHY_UNUSED]		= 0x0,
		[COMPHY_SATA_P1]	= 0x4,
		[COMPHY_SGMII_P1]	= 0x1,
		[COMPHY_HS_SGMII_P1]	= 0x1,
	},
	{	/* lane 1 */
		[COMPHY_UNUSED]		= 0x0,
		[COMPHY_SATA_P0]	= 0x4,
		[COMPHY_SGMII_P2]	= 0x1,
		[COMPHY_HS_SGMII_P2]	= 0x1,
	},
	{	/* lane 2 */
		[COMPHY_UNUSED]		= 0x0,
		[COMPHY_SATA_P0]	= 0x4,
		[COMPHY_SGMII_P0]	= 0x1,
		[COMPHY_HS_SGMII_P0]	= 0x1,
		[COMPHY_10GKR]		= 0x1,
	},
	{	/* lane 3 */
		[COMPHY_UNUSED]		= 0x0,
					     ...
					     enum mvebu_comphy_mode mode)
{
	...
	if (mode == COMPHY_10GKR)
		val |= MVEBU_COMPHY_SERDES_CFG0_GEN_RX(0xe) |
		       MVEBU_COMPHY_SERDES_CFG0_GEN_TX(0xe);
	if (mode == COMPHY_HS_SGMII_P0 || mode == COMPHY_HS_SGMII_P1 ||
	    mode == COMPHY_HS_SGMII_P2)
		val |= MVEBU_COMPHY_SERDES_CFG0_GEN_RX(0x8) |
		       MVEBU_COMPHY_SERDES_CFG0_GEN_TX(0x8) |
		       MVEBU_COMPHY_SERDES_CFG0_HALF_BUS;
	else if (mode == COMPHY_SGMII_P0 || mode == COMPHY_SGMII_P1 ||
				  ...
				  enum mvebu_comphy_mode mode)
{
	...
	do {
		mdelay(1);
		val = readl(priv->base + MVEBU_COMPHY_SERDES_STATUS0(lane->id));
	} while (timeout-- && !(val & (MVEBU_COMPHY_SERDES_STATUS0_RX_PLL_RDY |
	...
	do {
		mdelay(1);
		val = readl(priv->base + MVEBU_COMPHY_SERDES_STATUS0(lane->id));
	} while (timeout-- && !(val & MVEBU_COMPHY_SERDES_STATUS0_RX_INIT));
	if (!timeout)
		return -ETIMEDOUT;
...
static int mvebu_comphy_set_mode(struct phy *phy, enum phy_mode phy_mode)
{
	struct mvebu_comphy_lane *lane = phy_get_drvdata(phy);
	struct mvebu_comphy_priv *priv = lane->priv;
	enum mvebu_comphy_mode mode = phy_mode;
	int ret;
	u32 val;

	if (!mvebu_comphy_modes[lane->id][mode])
		return -EINVAL;

	regmap_read(priv->regmap, MVEBU_COMPHY_SELECTOR, &val);
	val &= ~(0xf << MVEBU_COMPHY_SELECTOR_PHY(lane->id));
	val |= mvebu_comphy_modes[lane->id][mode] <<
	       MVEBU_COMPHY_SELECTOR_PHY(lane->id);
	regmap_write(priv->regmap, MVEBU_COMPHY_SELECTOR, val);

	switch (mode) {
	case COMPHY_SGMII_P0:
	case COMPHY_SGMII_P1:
	case COMPHY_SGMII_P2:
	case COMPHY_HS_SGMII_P0:
	case COMPHY_HS_SGMII_P1:
	case COMPHY_HS_SGMII_P2:
		ret = mvebu_comphy_set_mode_sgmii(phy, mode);
		break;
	case COMPHY_10GKR:
		ret = mvebu_comphy_set_mode_10gkr(phy, mode);
		break;
	default:
		return -ENOTSUPP;
	}
				      ...
				      struct of_phandle_args *args)
{
	...
	if (WARN_ON(args->args[0] >= MVEBU_COMPHY_LANES))
		return ERR_PTR(-ENODEV);

	return priv->phys[args->args[0]];
}
