...
struct mvpp2 {
	...
	/* On PPv2.2, some port control regiters are located into the system
	 * controller space. These registers are accessible through syscon.
	 */
...
static void mvpp22_gop_init_xaui(struct mvpp2_port *port)
{
	struct mvpp2 *priv = port->priv;
	u32 val;

	regmap_read(priv->sysctrl_base, GENCONF_SD1_CTRL1, &val);
	val |= GENCONF_SD1_CTRL1_XAUI_EN;
...
static void mvpp22_gop_unmask_irq(struct mvpp2_port *port)
{
	...
	if (port->gop_id == 0) {
		...
		val = readl(port->base + MVPP22_XLG_EXT_INT_MASK);
			val |= MVPP22_XLG_EXT_INT_MASK_XLG |MVPP22_XLG_EXT_INT_MASK_GIG;
...
static void mvpp22_gop_mask_irq(struct mvpp2_port *port)
{
	...
	if (port->gop_id == 0) {
		/* Disable the XLG/GIG irqs for this port */
		val = readl(port->base + MVPP22_XLG_INT_MASK);
		val &= ~MVPP22_XLG_INT_MASK_LINK;
		writel(val, port->base + MVPP22_XLG_INT_MASK);
	...
	/* Disable the GMAC link status irq for this port */
	val = readl(port->base + MVPP22_GMAC_INT_MASK);
	val &= ~MVPP22_GMAC_INT_MASK_LINK_STAT;
	writel(val, port->base + MVPP22_GMAC_INT_MASK);
...
static void mvpp2_port_mii_gmac_configure_mode(struct mvpp2_port *port)
{
	...
	/* configure the GMAC to SGMII mode */
...
/* Link status per-port interrupt */
static irqreturn_t mvpp2_link_status_isr(int irq, void *dev_id)
{
	...
	if (val & MVPP22_GMAC_INT_STAT_LINK)
		pr_err("Link event from GMAC irq (%d)\n", port->id);

	val = readl(port->base + MVPP22_XLG_INT_STAT);
	if (val & MVPP22_XLG_INT_STAT_LINK)
		pr_err("Link event from XLG irq (%d)\n", port->id);
...
static void mvpp2_link_event(struct net_device *dev)
{
	...
	if (phydev->link && port->phy_interface != phydev->interface &&
	    port->comphy) {
...
static int mvpp2_open(struct net_device *dev)
{
	...
	if (port->irq_link) {
		err = request_irq(port->irq_link, mvpp2_link_status_isr, 0,
				  dev->name, port);
		if (err) {
			netdev_err(port->dev,
				   "cannot request link status IRQ %d\n",
				   port->irq);
			goto err_free_irq;
		}
	}
			    ...
			    struct mvpp2 *priv)
{
	...
	/* optional link status irq */
	port->irq_link = irq_of_parse_and_map(port_node, 1);
	if (port->irq_link <= 0)
		port->irq_link = 0;
...
err_free_irq:
	if (port->irq_link)
		irq_dispose_mapping(port->irq_link);
